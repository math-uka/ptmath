// 核心功能：物件建立、編輯、歷史記錄、拖曳與繪圖等（性能優化版）
(function() {
    // 材質快取
...
function getSurfaceMaterial(color, mode, isSelected = false, texture = null) {
    // 移动端模式：忽略光照和纹理，直接使用纯色材质（提高性能，且物体始终可见）
    if (window.isMobileMode) {
        if (isSelected) {
            // 选中时使用亮红色，不透明，方便识别
            return new THREE.MeshBasicMaterial({ color: 0xff4444 });
        }
        // 普通物体使用传入的固有色（无光照影响）
        return new THREE.MeshBasicMaterial({ color: color });
    }
    
    // 桌面版原逻辑（保留光照、纹理、半透明、线框模式等）
    if (isSelected) {
   ...
   const cacheKey = `${color}_${mode}_${texture ? texture.uuid : 'null'}_${window.isMobileMode}`;
    ...
    function loadFromHistory(index) {
        ...
        updateCount();
        if (window.updateCollisionGrid) window.updateCollisionGrid();
        recordLastSelection();
    ...
    else if (type === 'sphere') geo = new THREE.SphereGeometry(baseSize * 0.5, 16, 12);
    else if (type === 'cone') geo = new THREE.ConeGeometry(baseSize * 0.5, baseSize * 1.4, 16, 1);
    else if (type === 'pyramid') { const r = baseSize / Math.sqrt(2); geo = new THREE.ConeGeometry(r, baseSize, 4); geo.rotateY(Math.PI / 4); }
    else if (type === 'cylinder') geo = new THREE.CylinderGeometry(baseSize * 0.5, baseSize * 0.5, baseSize, 16, 1);
    else if (type === 'halfCylinder') geo = new THREE.CylinderGeometry(baseSize * 0.5, baseSize * 0.5, baseSize, 16, 1, false, 0, Math.PI);
    ...
    // 如果没有指定位置，则自动计算位置（优先在选中的物体右侧）
    if (!pos) {
        pos = getNextPositionForNewObject(type);
    }
...
function getNextPositionForNewObject(type) {
    // 1. 如果有选中的物体，在其右侧生成
    let referenceObj = null;
    if (selectedObjects.length > 0) {
        // 取最后一个选中的物体
        referenceObj = selectedObjects[selectedObjects.length - 1];
    } else if (selected) {
        referenceObj = selected;
    }
    
    if (referenceObj) {
        const refPos = referenceObj.position.clone();
        const newPos = refPos.clone();
        // 默认右侧：X + baseSize
        newPos.x += baseSize;
        // 保持同样的Y坐标（对于非地板类）
        if (type !== 'floor') {
        ...
        // 检查该位置是否被占
        let occupied = objects.some(obj => obj.position.equals(newPos));
        let tryOffset = 1;
        while (occupied && tryOffset < 10) {
            // 尝试继续右移，每次移动baseSize
            newPos.x += baseSize;
            occupied = objects.some(obj => obj.position.equals(newPos));
            tryOffset++;
        }
        if (!occupied) {
            return newPos;
    ...
    function createPlayerModel(pos = null, color = 0xff0000, rotation = { x: 0, y: 0, z: 0 }, saveHistory = true) {
        ...
        group.add(rightArm);
        const legGeo = new THREE.CylinderGeometry(0.4 * scale, 0.4 * scale, 1.5 * scale, 4);
...
function addPlayerModel() {
    ...
    const basePos = new THREE.Vector3(0, 0.75, 0);
    let finalPos = null;

    // 1. 檢查預設位置
    if (!checkCollision(basePos)) {
        finalPos = basePos;
    } else {
        // 2. 嘗試周圍偏移 (右、左、前、後、斜角)
        const offsets = [
        ...
        for (let [dx, dz] of offsets) {
            const testPos = new THREE.Vector3(basePos.x + dx, basePos.y, basePos.z + dz);
        ...
        // 3. 若仍碰撞，直接放到較遠的空曠位置
        if (!finalPos) {
            finalPos = new THREE.Vector3(5, 0.75, 5);
        }
    }

    playerModel = createPlayerModel(finalPos);
    ...
    function startDragObject(obj, mouseWorldPos) {
        ...
        const proxySize = new THREE.Vector3(1, 1, 1);
        if (obj.type === 'polygon') {
            const min = new THREE.Vector3(Infinity, Infinity, Infinity);
            const max = new THREE.Vector3(-Infinity, -Infinity, -Infinity);
            obj.vertices.forEach(v => { min.min(v); max.max(v); });
            proxySize.copy(max.sub(min));
            proxySize.y = 0.1;
        } else if (obj.type === 'player') {
            proxySize.set(0.8, 1.8, 0.8);
        } else if (obj.type === 'objgroup') {
        ...
        const proxyGeo = new THREE.BoxGeometry(proxySize.x, proxySize.y, proxySize.z);
        dragProxy = new THREE.Mesh(proxyGeo, new THREE.MeshPhongMaterial({ color: 0x888888, transparent: true, opacity: 0.4 }));
...
function toggleMobileMode() {
    // 切換標誌
    window.isMobileMode = !window.isMobileMode;
    ...
    if (window.isMobileMode) {
        ...
        if (!wireframeVisible) {
            ...
            if (wireBtn) wireBtn.textContent = '🔲 隱藏線框';
        }
        ...
        // 可選：自動隱藏側邊欄（提升可視區域）
        const sidebar = document.getElementById('sidebar');
        if (sidebar && !sidebar.classList.contains('hidden-sidebar') && uiVisible) {
            sidebar.classList.add('hidden-sidebar');
            uiVisible = false;
        }
        
        showStatus('已切換至平板模式 (效能優化：純線框 + 無陰影)');
    } else {
        // ========== 電腦模式：還原完整效果 ==========
        // 恢復陰影渲染
        ...
        if (directionArrows) directionArrows.forEach(a => a.visible = true);
        
        // 恢復不透明表面 + 線框模式（displayMode = 0）
        if (displayMode !== 0) {
            displayMode = 0;
            const modeBtn = document.getElementById('toggleDisplayMode');
        ...
        if (!shadowsEnabled) {
            shadowsEnabled = true;
            const shadowBtn = document.getElementById('toggleShadows');
            if (shadowBtn) shadowBtn.textContent = '🌓 隱藏陰影';
            // 重新對所有物件啟用陰影
            objects.forEach(o => {
                if (o.type === 'player') {
                    o.group.traverse(child => { if (child.isMesh) { child.castShadow = true; child.receiveShadow = true; } });
                } else if (o.type === 'objgroup') {
                    o.group.traverse(child => { if (child.isMesh) { child.castShadow = true; child.receiveShadow = true; } });
                } else if (o.mesh) {
        ...
        showStatus('已切換至電腦模式 (完整效果：不透明表面 + 線框 + 陰影)');
    }
    
    // 根據新的顯示模式更新所有物件的可見性與材質
    objects.forEach(obj => {
        // 更新材質（getSurfaceMaterial 會根據 isMobileMode 返回正確材質）
        refreshObjectMaterial(obj);
        
        // 更新網格可見性
        if (obj.type !== 'player' && obj.type !== 'objgroup') {
            obj.mesh.visible = (displayMode !== 2);
            obj.wire.visible = wireframeVisible;
            if (obj.diagWire) obj.diagWire.visible = wireframeVisible;
        } else if (obj.type === 'objgroup' && obj.boundingWire) {
            obj.boundingWire.visible = wireframeVisible;
        }
        
        // 更新陰影屬性（根據新的 shadowsEnabled）
        if (obj.type === 'player') {
            obj.group.traverse(child => { if (child.isMesh) { child.castShadow = shadowsEnabled; child.receiveShadow = shadowsEnabled; } });
        } else if (obj.type === 'objgroup') {
            obj.group.traverse(child => { if (child.isMesh) { child.castShadow = shadowsEnabled; child.receiveShadow = shadowsEnabled; } });
        } else if (obj.mesh) {
            obj.mesh.castShadow = shadowsEnabled;
            obj.mesh.receiveShadow = shadowsEnabled;
        }
    });
