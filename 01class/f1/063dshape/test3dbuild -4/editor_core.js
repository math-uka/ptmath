...
function getSurfaceMaterial(color, mode, isSelected = false, texture = null) {
    ...
    const cacheKey = `${color}_${mode}_${texture ? texture.uuid : 'null'}`;
...
function getNextPositionForNewObject(type) {
    ...
    function checkCollision(proposedPos) {
        ...
        const cellSize = collisionGrid.cellSize;
        const cellX = Math.floor(proposedPos.x / cellSize);
        const cellZ = Math.floor(proposedPos.z / cellSize);
        const nearbyKeys = [
            `${cellX},${cellZ}`, `${cellX-1},${cellZ}`, `${cellX+1},${cellZ}`,
            `${cellX},${cellZ-1}`, `${cellX},${cellZ+1}`,
            `${cellX-1},${cellZ-1}`, `${cellX-1},${cellZ+1}`, `${cellX+1},${cellZ-1}`, `${cellX+1},${cellZ+1}`
        ];
        for (let key of nearbyKeys) {
            const cellObjs = collisionGrid.map.get(key);
            if (cellObjs) {
                for (let obj of cellObjs) {
